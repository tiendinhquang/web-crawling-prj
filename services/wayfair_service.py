...
class WayfairService:
    ...
    def _get_variation_key(
        self,
        variation: Union[Dict[str, Any], str]
    ) -> str:
        """
    ...
    def get_failed_product_variations(
        ...
        # Handle variations
        all_key_map = {self._get_variation_key(v): v for v in all_variations}
        all_keys = set(all_key_map.keys())
        success_keys = set(self._get_variation_key(v) for v in success_variations)
        failed_keys = all_keys - success_keys
